...
struct Problem
    ...
    ρ::Float64
    b::Float64
    Problem(name; E=2e5,ν=0.3,ρ=7.85e-9,b=1,type="PlaneStress") = new(name,type,E,ν,ρ,b)
end
...
function revertTriangles(problem)
    gmsh.model.setCurrent(problem.name)
    elemTypes, elemTags, elemNodeTags = gmsh.model.mesh.getElements(2, -1)
    for i in 1:length(elemTypes)
        elementName, dim, order, numNodes::Int64, localNodeCoord, numPrimaryNodes = gmsh.model.mesh.getElementProperties(elemTypes[i])
        if numPrimaryNodes == 5
            gmsh.model.mesh.reverseElements(elemTags[i])
        end
    end
end
